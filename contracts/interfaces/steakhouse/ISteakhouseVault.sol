 ...
 */
interface ISteakhouseVault is IERC20 {
    /// @dev Errors
    error Deadline();
    ...
    function withdrawalRequest(address user) external view returns (WithdrawalRequest memory request);
    ...
    function pendingWithdrawers() external view returns (address[] memory users);

    /// @notice Returns an array of addresses with pending withdrawal requests.
    /// @param limit The maximum number of users to return.
    /// @param offset The number of users to skip before returning.
    /// @return users An array of addresses with pending withdrawal requests.
    function pendingWithdrawers(uint256 limit, uint256 offset) external view returns (address[] memory users);
    ...
    function externalCall(address to, bytes calldata data) external returns (bool success, bytes memory response);
    ...
    /// @dev Checks permissions using the validator from the configurator.
    function delegateCall(address to, bytes calldata data) external returns (bool success, bytes memory response);
    ...
    function analyzeRequest(ProcessWithdrawalsStack memory s, WithdrawalRequest memory request)
        external
        pure
        returns (bool processingPossible, bool withdrawalPossible, uint256[] memory expectedAmounts);
    ...
    function processWithdrawals(address[] memory users) external returns (bool[] memory statuses);
     ...
     */
    event ExternalCall(address indexed to, bytes data, bool success, bytes response);
     ...
     * @param response The response data of the call.
     */
    event DelegateCall(address indexed to, bytes data, bool success, bytes response);
     ...
     * @param lpAmount The amount of LP tokens minted.
     */
    event DepositCallback(address indexed callback, uint256[] amounts, uint256 lpAmount);
     ...
     */
    event EmergencyWithdrawal(address indexed from, WithdrawalRequest request, uint256[] amounts);
